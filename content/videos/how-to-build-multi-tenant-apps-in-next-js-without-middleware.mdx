...
Here's the videos subdomain page implementation using standard Next.js pages:

```tsx title="app/videos/page.tsx"
// Using a custom Page helper for type-safe routing
// You can also use standard Next.js: export default function VideosPage() { ... }
...
// Using a custom Page helper - alternatively use: export default async function SubdomainPage({ params })
export default Page.create({
  path: "/[subdomain]",
  name: "catch-all-subdomain",
}).page(async ({ getPathParams }) => {
  const { subdomain } = await getPathParams(); // [!code highlight]
  // In standard Next.js: const { subdomain } = await params
