...
type PluginConfig = {
  /** Base directory for app routes, relative to project root */
  appDir?: string;
};
...
const ERROR_CODE_DIRECT = 100_001;
const ERROR_CODE_REEXPORT = 100_002;

/** Error messages */
const ERROR_MESSAGE_DIRECT = (actualPath: string, expectedPath: string) => `Page.create path "${actualPath}" doesn't match expected "${expectedPath}" for this file location.`;
const ERROR_MESSAGE_REEXPORT = (actualPath: string, expectedPath: string) => `Exported page has path "${actualPath}" but this file location expects "${expectedPath}".`;
 ...
 */
function isPageFile(filePath: string, appDir: string): boolean {
  const normalizedPath = filePath.replace(/\\/g, "/");
 ...
 */
function isPageCreateCall(expr: ts.Expression, typescript: typeof ts): boolean {
 ...
 */
function extractPathFromOptions(arg: ts.ObjectLiteralExpression, typescript: typeof ts): PathResult | null {
 ...
 */
function findPageCreatePath(sourceFile: ts.SourceFile, typescript: typeof ts): PathResult | null {
 ...
 */
function extractPathFromPageFnType(type: ts.Type, typeChecker: ts.TypeChecker, location: ts.Node): string | null {
  // Try to find PageFnBrand property
  const brandSymbol = type.getProperty("__brand_PageFn" as string) || type.getProperties().find((p) => p.name.includes("PageFnBrand"));
 ...
 */
function findDefaultExportIdentifierPath(sourceFile: ts.SourceFile, typeChecker: ts.TypeChecker, typescript: typeof ts): PathResult | null {
 ...
 */
function findReexportedPagePath(sourceFile: ts.SourceFile, typeChecker: ts.TypeChecker, typescript: typeof ts): PathResult | null {
  for (const statement of sourceFile.statements) {
 ...
 */
function createMismatchDiagnostic(options: DiagnosticOptions): ts.Diagnostic {
 ...
 */
function getPageRouteDiagnostics(fileName: string, languageService: ts.LanguageService, appDir: string, typescript: typeof ts): ts.Diagnostic[] {
